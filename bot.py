...
async def send_random_from_category(update: Update, context: ContextTypes.DEFAULT_TYPE): category = update.message.text[1:].lower() user_id = update.effective_user.id all_items = [] for uploads in user_uploads.values(): all_items += [f for f in uploads if f["type"] == category]
...
for uploads in user_uploads.values():
    all_items += [f for f in uploads if f["type"] == file_type]
...
async def handle_file(update: Update, context: ContextTypes.DEFAULT_TYPE): user_id = update.effective_user.id if user_id not in ADMIN_IDS or not user_states.get(user_id, False): return
