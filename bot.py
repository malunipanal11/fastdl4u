import os import json import logging import random import atexit from fastapi import FastAPI, Request from telegram import Update, BotCommand, InlineKeyboardMarkup, InlineKeyboardButton from telegram.ext import Application, CommandHandler, MessageHandler, ContextTypes, CallbackQueryHandler, filters from typing import Dict, List import httpx

--- Environment Configuration ---

TOKEN = os.getenv("BOT_TOKEN") WEBHOOK_URL = os.getenv("WEBHOOK_URL") or os.getenv("WEBHOOK_DOMAIN", "") + "/webhook" GOFILE_TOKEN = os.getenv("GOFILE_TOKEN") ADMIN_IDS = list(map(int, os.getenv("ADMIN_IDS", "").split(",")))
...
try: with open("uploads.json") as f: user_uploads.update(json.load(f)) except FileNotFoundError: pass

@atexit.register def save_data(): with open("uploads.json", "w") as f: json.dump(user_uploads, f)
...
async def upload_to_gofile(file_bytes: bytes, filename: str) -> str: try: async with httpx.AsyncClient(timeout=30.0) as client: server_resp = await client.get("https://api.gofile.io/getServer") server_data = server_resp.json() if server_data["status"] != "ok": raise Exception(f"Failed to get server: {server_data}") server = server_data["data"]["server"]
...
files = {"file": (filename, file_bytes)}
        ...
        logger.info(f"Gofile upload success: {upload_data}")
...
async def send_random_from_category(update: Update, context: ContextTypes.DEFAULT_TYPE): category = update.message.text[1:].lower() user_id = update.effective_user.id all_items = [f for uploads in user_uploads.values() for f in uploads if f["type"] == category] if not all_items: await update.message.reply_text(f"❌ No {category} uploaded yet.") return file = random.choice(all_items) keyboard = [[ InlineKeyboardButton("▶️ Play", url=file["url"]), InlineKeyboardButton("⬇️ Download", url=file["url"]) ]] if user_id in ADMIN_IDS: keyboard[0].append(InlineKeyboardButton("🗑️ Delete", callback_data=f"delete:{file['url']}")) keyboard[0].append(InlineKeyboardButton("📤 Send", callback_data=f"send:{file['url']}")) await update.message.reply_text(f"📂 Random {category}:", reply_markup=InlineKeyboardMarkup(keyboard))

async def get_by_serial(update: Update, context: ContextTypes.DEFAULT_TYPE): if not context.args: await update.message.reply_text("❌ Usage: /get <serial> (e.g., img1, video2)") return serial = context.args[0].lower() for full_type, short in type_map.items(): if serial.startswith(short): try: index = int(serial[len(short):]) - 1 except ValueError: continue all_items = [f for uploads in user_uploads.values() for f in uploads if f["type"] == full_type] if index >= len(all_items): await update.message.reply_text("❌ File not found.") return file = all_items[index] keyboard = [[ InlineKeyboardButton("▶️ Play", url=file["url"]), InlineKeyboardButton("⬇️ Download", url=file["url"]) ]] if update.effective_user.id in ADMIN_IDS: keyboard[0].append(InlineKeyboardButton("🗑️ Delete", callback_data=f"delete:{file['url']}")) keyboard[0].append(InlineKeyboardButton("📤 Send", callback_data=f"send:{file['url']}")) await update.message.reply_text(f"📦 Here is {serial}:", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown") return await update.message.reply_text("❌ Invalid serial format.")
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE): query = update.callback_query await query.answer() data = query.data if data.startswith("delete:"): url = data.split(":", 1)[1] for uploads in user_uploads.values(): for i, item in enumerate(uploads): if item["url"] == url: del uploads[i] await query.edit_message_text("🗑️ File deleted.") return await query.edit_message_text("❌ File not found.") elif data.startswith("send:"): url = data.split(":", 1)[1] await query.message.reply_text(f"📤 File URL: {url}")

--- File Upload Handler ---

async def handle_file(update: Update, context: ContextTypes.DEFAULT_TYPE): if not update.message: return user_id = update.effective_user.id if user_id not in ADMIN_IDS or not user_states.get(user_id, False): return
...
if tg_file:
    ...
    try:
        ...
        logger.info(f"Uploading file: {filename}, size={len(file_bytes)} bytes")
...
@app.on_event("startup") async def startup_event(): await application.initialize() await application.bot.set_webhook(WEBHOOK_URL) commands = [ BotCommand("start", "Start the bot"), BotCommand("images", "Random image"), BotCommand("videos", "Random video"), BotCommand("audios", "Random audio"), BotCommand("files", "Random file"), BotCommand("texts", "Random text"), BotCommand("get", "Get by serial"), BotCommand("list", "List your uploads") ] if ADMIN_IDS: commands += [BotCommand("add", "Enter upload mode"), BotCommand("done", "Exit upload mode")] await application.bot.set_my_commands(commands) logger.info("✅ Webhook and commands registered.")

@app.post("/webhook") async def telegram_webhook(req: Request): update_dict = await req.json() logger.info(f"🔔 Telegram webhook update: {update_dict}") update = Update.de_json(update_dict) await application.process_update(update) return {"status": "ok"}
